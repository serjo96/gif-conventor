...
const worker = new Worker<JobData>('video-conversion', async (job: Job<JobData>) => {
  ...
  try {
    ...
    await job.log(`Failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`);
